...
function makeTable(opts, filteredList) {
  ...
  if (!opts.pagination) table(data, { targetDiv: opts.targetDiv })
  var allRows = data.length
  var totalPages = Math.ceil(allRows / opts.pagination)
  var pageParam = window.location.hash.match(/#page(.*)/)
  var currentPage = pageParam ? parseInt(pageParam[1].split("?")[0]) : 1
  ...
  if (opts.data.length > opts.pagination) writePreNext(opts.tableDiv, currentPage, totalPages, data, opts.pagination)
...
function setPagClicks(data, tableId, currentPage, pagination, totalPages) {
  $(document).on("click", (".pagination-next-" + tableId), function() {
    ...
    table(currentRows, { templateID: tableId, tableDiv: "#" + tableId })
    setPreNext("#" + tableId, currentPage, totalPages)
})
...
function  setPreNext(targetDiv, currentPage, totalPages, data, pagination) {
...
function  writePreNext(targetDiv, currentPage, totalPages, data, pagination) {
  var tableId = targetDiv.slice(1)
  setPreNext(targetDiv, currentPage, totalPages, data, pagination)
